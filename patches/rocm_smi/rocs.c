...
#define ADDENERGY_PRINT_ROCM_SUPPORTED_FUNCTIONS 0
...
#if ADDENERGY_PRINT_ROCM_SUPPORTED_FUNCTIONS
            printf("ROCM_SUPPORTED_FUNCTIONS: device=%d, supported_func=%s\n", dev, v_name.name);
#endif
...
/* If we have the symbol, assume it's supported or attempt a quick test */
   if (rsmi_dev_energy_count_get_p) {
    for (dev = 0; dev < device_count; dev++) {
        uint64_t test_power = 0ULL;
        float    test_res   = 0.0f;
        uint64_t test_ts    = 0ULL;

        rsmi_status_t st = rsmi_dev_energy_count_get_p(dev, &test_power, &test_res, &test_ts);

        /*
         * Some devices might return RSMI_STATUS_NOT_SUPPORTED if truly unsupported.
         * If st==RSMI_STATUS_SUCCESS or st==RSMI_STATUS_INVALID_ARGS or st==RSMI_STATUS_PERMISSION, 
         * treat it as "supported." Adjust as needed for your environment.
         */
        if (st == RSMI_STATUS_SUCCESS ||
            st == RSMI_STATUS_INVALID_ARGS ||
            st == RSMI_STATUS_PERMISSION) 
        {
            /* We will add exactly 1 event for this device. */
            (*count)++;
        }
    }
}
...
if (rsmi_dev_energy_count_get_p) {
    for (dev = 0; dev < device_count; dev++) {
        ...
        if (st == RSMI_STATUS_SUCCESS ||
            ...
            st == RSMI_STATUS_PERMISSION) 
        {
            /* Construct a name for the event: "energy_count:device=X" */
            char tmp_name[PAPI_MAX_STR_LEN];
            snprintf(tmp_name, sizeof(tmp_name),
                     "energy_count:device=%d", dev);

            /* Fill in ntv_event_t */
            events[events_count].id        = events_count;
            events[events_count].name      = strdup(tmp_name);
            events[events_count].descr     = strdup("Accumulated GPU energy, in microjoules (µJ).");
            events[events_count].device    = dev;
            events[events_count].variant   = -1; /* no variant here */
            events[events_count].subvariant= -1;
            events[events_count].offset    = test_power;
            events[events_count].mode      = ROCS_ACCESS_MODE__READ;  /* read-only */
            events[events_count].open_func_p  = get_open_func("rsmi_dev_energy_count_get");
            events[events_count].close_func_p = get_close_func("rsmi_dev_energy_count_get");
            events[events_count].start_func_p = get_start_func("rsmi_dev_energy_count_get");
            events[events_count].stop_func_p  = get_stop_func("rsmi_dev_energy_count_get");
            events[events_count].access_func_p= get_access_func("rsmi_dev_energy_count_get");

            /* Add it to the hash table so that rocs_evt_name_to_code() can find it */
            htable_insert(htable, events[events_count].name, &events[events_count]);
            events_count++;
        }
    }
...
access_rsmi_dev_energy_count(rocs_access_mode_e mode, void *arg)
{
     ...
     */
    long double total_uJ = (long double)(raw_count - event->offset) * (long double)resolution;
    if (total_uJ > (long double)INT64_MAX) {
        /* clamp to avoid overflow */
        total_uJ = (long double)INT64_MAX;
    }
