...
function setup_scorep_env() {
    ...
    export PATH="/opt/rocm-$ROCM_VERSION/llvm/lib:$PATH"
    ...
    if [ -z "$LIBRARY_PATH" ]; then
        export LIBRARY_PATH="$PAPI_ROCM_ROOT/lib:/opt/rocm-$ROCM_VERSION/lib:/opt/rocm-$ROCM_VERSION/llvm/lib"
    else
        export LIBRARY_PATH="$PAPI_ROCM_ROOT/lib:/opt/rocm-$ROCM_VERSION/lib:/opt/rocm-$ROCM_VERSION/llvm/lib:$LIBRARY_PATH"
    fi
    if [ -z "$LD_LIBRARY_PATH" ]; then
        export LD_LIBRARY_PATH="$PAPI_ROCM_ROOT/lib:/opt/rocm-$ROCM_VERSION/lib:/opt/rocm-$ROCM_VERSION/llvm/lib"
    else
        export LD_LIBRARY_PATH="$PAPI_ROCM_ROOT/lib:/opt/rocm-$ROCM_VERSION/lib:/opt/rocm-$ROCM_VERSION/llvm/lib:$LD_LIBRARY_PATH"
    fi
    # if [ -z "$C_INCLUDE_PATH" ]; then
...
export ROCM_VERSION=$(echo "$ROCM_VERSIONS" | tail -n 1)
...
if [ $BUILD_LLVM -eq 1 ]; then
    ...
    cmake -S llvm \
        ...
        -DLLVM_ENABLE_RUNTIMES="libc;libcxx;libcxxabi;openmp;offload;libunwind;compiler-rt" \
...
if [ $BUILD_SCOREP -eq 1 ]; then
    if [ ! -d "scorep-9.2" ]; then
        log_info "scorep-9.2 directory not found, downloading..."
        wget https://perftools.pages.jsc.fz-juelich.de/cicd/scorep/tags/scorep-9.2/scorep-9.2.tar.gz
        if [ $? -ne 0 ]; then
            log_error "Failed to download Score-P 9.2."
            exit 1
        fi
        tar xf scorep-9.2.tar.gz
    fi
    if [ $? -ne 0 ]; then
        log_error "Failed to extract Score-P 9.2."
        exit 1
    fi
    cd scorep-9.2
    rm -Rf _build
    ...
    ../configure \
        ...
        # --with-llvm=/opt/rocm-$ROCM_VERSION/llvm/ \
        # --with-papi="$PAPI_ROOT" \
        # --with-librocm_smi64-include=/opt/rocm-$ROCM_VERSION/rocm_smi/include \
